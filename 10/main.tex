...
\cs_generate_variant:Nn \regex_split:nnN { nVN }
    ...
    \foreach \x in {0.000,0.002,0.004,0.006,0.008}
    {
    ...
    \ior_str_map_variable:NNn \g__csv_ior \l_tmpa_str {
        ...
        \node[scale = .5,inner~sep=0pt] (abr-\int_use:N\l_xaxis_int) at (\int_use:N \l_xaxis_int, \seq_item:Nn\l_tmpa_seq{5}){\tikz{\draw[yellow,line~width = .2pt](60\str_use:N\c_colon_str 1pt) -- (180\str_use:N\c_colon_str 1pt) -- (300\str_use:N\c_colon_str 1pt) -- cycle;}};
    ...
    \int_step_inline:nn{\int_use:N \l_xaxis_int - 1} {
        \draw[yellow,line~width = .1pt,line~cap=round] (abr-#1.center) -- (abr-\int_eval:n{#1+1}.center);
    }
    \int_step_inline:nn{\int_use:N \l_xaxis_int - 1} {
    ...
    \draw[line~width = .1pt] (-5,-0.002) rectangle (112,0.01);
    \foreach \x in {0,1,...,107} {
        \node[scale = .3,below = 8pt] at (\x,-.002) {\x};
        \draw[line~width = .1pt] (\x,-.002) --++ (0,-2pt);
    }
    \node[font = \bfseries,scale = .3,rotate = 90] at (-10,0.004) {displacements_core~mm};
    \node[font = \bfseries,scale = .3] at (53.5,-.0035) {number};
    \node[font = \bfseries,scale = .3] at (53.5,0.0105) {displacements_core~Graph};
    \begin{pgfonlayer}{lable}
        \node[draw,line~width = .1pt,anchor = south~west,inner~sep = 1pt,fill = white,opacity = .8] at (115,-0.002){
        \tikz[anchor=center] {
            ...
            \draw[line~width = .2pt,yellow] (0,0.001) -- (4,0.001);
            \node[] at (2,0.001) {\tikz{\draw[yellow,line~width = .2pt](60\str_use:N\c_colon_str 1pt) -- (180\str_use:N\c_colon_str 1pt) -- (300\str_use:N\c_colon_str 1pt) -- cycle;}};
            \node[anchor = west,scale = .3] at (4.5,0.001) {ABR};
